...
#[derive(StructOpt)]
#[structopt(
    name = "get-minidump-instructions",
    about = "Display instructions from a minidump"
)]
...
#[structopt(
    name = "dumpstack",
    about = "Display possible stack frames from a minidump"
)]
struct DumpStack {
    #[structopt(help = "Input minidump", parse(from_os_str))]
    minidump: PathBuf,
    ...
    #[structopt(
        short = "a",
        help = "Show stack addresses that don't resolve to functions"
    )]
    all: bool,
}
...
impl VCSFile for GitHubFile {
    ...
    fn annotate_url(&self, line: u64) -> String {
        format!(
            ...
            self.repo, self.rev, self.path, line
        )
    }
    fn as_local_filename(&self) -> String {
...
fn handle_symbol_paths(symbol_paths: Vec<PathBuf>) -> Result<Symbolizer, Error> {
    ...
    let (symbol_paths, symbol_urls) = if symbol_paths.is_empty() {
        ...
        let urls = symbol_paths
            .iter()
            .filter(|p| p.starts_with("http"))
            .filter_map(|p| p.to_str().map(str::to_owned))
            .collect();
        let paths = symbol_paths
            .into_iter()
            .filter(|p| !p.starts_with("http"))
            .collect();
        (paths, urls)
...
pub fn get_minidump_instructions() -> Result<(), Error> {
    ...
    info!(
        "Disassembling {} bytes starting at {:#x} of arch {:?}",
        memory.bytes.len(),
        memory.base_address,
        arch
    );
    disasm::disasm_bytes(
        &memory.bytes[ip as usize - memory.base_address as usize..],
        ip,
        arch,
...
pub fn dump_minidump_stack() -> Result<(), Error> {
    ...
    let memory = memory_list.memory_at_address(sp).ok_or_else(|| {
        format_err!(
            "Minidump doesn't contain a memory region that contains address {:#x}",
            sp
        )
    })?;
