...
//!
//! **Note: these type definitions may not be abi-compatible with the contents of a minidump,
//! for two reasons:**
//!
//! * Minidumps have the native endianess of the platform they were generated on, which
//!   is not necessarily the same endianess as the platform reading them.
//!
//! * Some minidump types need `repr(packed)` to have the exact right field offsets
//!   (and alignment of types is platform-specific in general anyway).
//!
//! Each type implements scroll's TryFromCtx (read), TryIntoCtx (write), and SizeWith traits 
//! which give you everything you need to read and write that type in a minidump 
//! (using [scroll::Pread][] and [scroll::Pwrite][]). 
//!
//! That said, the order of the fields *should* be correct, as well as
//! the sizes of all the primitive types. Sizes of aggregates may be wrong
//! because of padding/alignment differences, but [scroll::SizeWith][] will 
//! return the size of the value (in bytes) when stored in a minidump.
//!
//! ```
//! use scroll::ctx::SizeWith;
//! use scroll::{self, Pread, Pwrite};
//! use minidump_common::format::{CPU_INFORMATION, MINIDUMP_SYSTEM_INFO};
//!
//! fn main() -> Result<(), scroll::Error> {
//!   let input = MINIDUMP_SYSTEM_INFO {
//!        processor_architecture: 1,
//!        processor_level: 9999,
//!        processor_revision: 24,
//!        number_of_processors: 10,
//!        product_type: 0,
//!        major_version: 1,
//!        minor_version: 2,
//!        build_number: 3,
//!        platform_id: 12312345,
//!        csd_version_rva: 0,
//!        suite_mask: 0,
//!        reserved2: 0,
...
//!
//!   // Pick an endianess
//!   let endianess = scroll::Endian::Big;
//!
//!   // Get how much space in the buffer is needed with `size_with`
//!   let mut output = vec![0; MINIDUMP_SYSTEM_INFO::size_with(&endianess)];
//!   
//!   // Write with pwrite/gwrite (writes in the layout of a big-endian minidump).
//!   output.pwrite_with(&input, 0, endianess)?;
//!
//!   // Read back out with pread/gread (reads the layout of a big-endian minidump).
//!   let input2: MINIDUMP_SYSTEM_INFO = output.pread_with(0, endianess)?;
//!   
//!   // Check that data roundtripped correctly.
...
pub use crate::linux_errors::*;
pub use crate::mac_errors::*;
pub use crate::windows_errors::*;
