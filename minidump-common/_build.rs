macro_rules! make_enum {
    ($discriminants:ident, $names:ident => $($name:ident = $val:literal,)+) => {
        const $discriminants: &[u32] = &[$($val,)+];
        const $names: &[&str] = &[$(stringify!($name),)+];
    };
}

make_enum!(ERROR_NAMES, ERROR_VALUES =>
    ERROR_SUCCESS = 0,
...
make_enum!(STATUS_NAMES, STATUS_VALUES =>
...
fn write_phf<W: Write>(
    ...
    let hash_state = phf_generator::generate_hash(discriminants);
...
pub enum {name} {{"#
    ...
    for (name, discriminant) in names.iter().zip(discriminants.iter()) {
        writeln!(writer, "    {name} = {discriminant:#x?},")?;
    }
    writeln!(writer, "}}")?;

    writeln!(
        writer,
        "const {name}_KEY: phf_shared::HashKey = {:#x?};",
        hash_state.key
    )?;
    writeln!(writer, "const {name}_DISPS: &[(u32, u32)] = &[")?;
    for displacement in hash_state.disps {
        writeln!(writer, "    ({}, {}),", displacement.0, displacement.1)?;
    }
    writeln!(writer, "];")?;

    writeln!(writer, "const {name}_NAMES: &[&str] = &[")?;
    for idx in &hash_state.map {
        writeln!(writer, "    {:?},", names[*idx])?;
    }
    writeln!(writer, "];")?;

    writeln!(writer, "const {name}_VALUES: &[{name}] = &[")?;
    for idx in hash_state.map {
        writeln!(writer, "    {name}::{},", names[idx])?;
    }
    writeln!(writer, "];")?;
...
impl {name} {{
    pub fn from_u32(discriminant: u32) -> Option<Self> {{
        let hashes = phf_shared::hash(&discriminant, &{name}_KEY);
        let index = phf_shared::get_index(&hashes, {name}_DISPS, {name}_VALUES.len());

        let value = {name}_VALUES[index as usize];
...
impl fmt::Debug for {name} {{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {{
        ...
        let hashes = phf_shared::hash(&discriminant, &{name}_KEY);
        let index = phf_shared::get_index(&hashes, {name}_DISPS, {name}_NAMES.len());

        f.write_str({name}_NAMES[index as usize])
    }}
}}"#
    )
}
...
fn main() -> Result<()> {
    ...
    let mut writer = std::io::BufWriter::new(std::fs::File::create(format!(
        "{crate_dir}/src/errors/windows_gen.rs"
    ))?);
